...
import { Key, Sliders, ShieldCheck, Save, RefreshCw, Cpu, Database, Eye, EyeOff } from 'lucide-react';
    ...
    <div className="flex-1 flex flex-col gap-6 overflow-y-auto pr-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-panel p-6 flex flex-col gap-5">
          ...
          {/* Hybrid Weighting (Semantic vs BM25) */}
          <div className="flex flex-col gap-1.5">
            <div className="flex justify-between items-center text-xs font-medium text-slate-300">
              <span>Semantic Search Ratio</span>
              <span className="font-mono text-emerald-400 font-bold">{Math.round(settings.hybridWeightSemantic * 100)}%</span>
            </div>
            <p className="text-[11px] text-[var(--text-dim)]">
              BM25 Keyword Ratio: {Math.round((1 - settings.hybridWeightSemantic) * 100)}%
            </p>
            <input
              type="range"
              min="0.1"
              max="0.9"
              step="0.05"
              value={settings.hybridWeightSemantic}
              onChange={(e) => setSettings({ ...settings, hybridWeightSemantic: parseFloat(e.target.value) })}
      ...
      <div className="flex justify-end pt-2">
        <button
          onClick={handleSave}
          className="flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-semibold text-sm shadow-lg shadow-purple-500/20 transition-all cursor-pointer"
        >
